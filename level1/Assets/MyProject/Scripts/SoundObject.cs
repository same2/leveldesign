...
public class SoundObject : MonoBehaviour {
	...
	void Update () {
		...
		if (isHolding==true)
		{
			item.GetComponent<Rigidbody>().gameObject.tag = "Untagged";
			...
			if (Input.GetMouseButtonDown (1)) {
				...
				isHolding = false;
				item.GetComponent<Rigidbody> ().gameObject.tag = "Box";
			} 
		}
		else
		{
	...
	{
		isHolding = false;
		if ((collision.gameObject.tag == "Floor") && item.GetComponent<Rigidbody> ().gameObject.tag == "Box" ){
