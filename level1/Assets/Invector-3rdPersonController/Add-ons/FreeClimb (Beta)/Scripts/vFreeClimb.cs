...
namespace Invector.CharacterController.Actions
{
    ...
    public class vFreeClimb : vMonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            {
                input = new Vector3(TP_Input.horizontalInput.GetAxis(), TP_Input.verticallInput.GetAxis());
                ClimbHandle();
                ClimbJumpHandle();
                ClimbUpHandle();
            }
        ...
        protected virtual void ClimbHandle()
        {
            ...
            if (/*(TP_Input.cc.isJumping || !TP_Input.cc.isGrounded)*/ !dragInfo.inDrag)
            {
            ...
            if (Physics.Raycast(handTargetPosition, transform.forward, out hit, 0.7f, draggableWall) && dragInfo.canGo)
            {
                ...
                if (enterExitInput.GetButtonDown() && dragInfo.inDrag && input.magnitude == 0 && Time.time > (oldInput + 0.5f))
                    ExitClimb();
                else if (dragInfo.canGo && (enterExitInput.GetButton() || TP_Input.cc.input.y > 0.1f) && !dragInfo.inDrag && Time.time > (oldInput + 0.5f))
                    EnterClimb();
            }
            ClimbMovement();
        }
        ...
        protected virtual void ClimbMovement()
        {
            ...
            if (input.y < 0 && Physics.Raycast(transform.position + Vector3.up * (TP_Input.cc._capsuleCollider.height * 0.5f), Vector3.down, TP_Input.cc._capsuleCollider.height, TP_Input.cc.groundLayer))
            {
                ExitClimb();
            }
        }
        ...
        protected virtual bool CheckCanMoveClimb()
        {
            ...
            climbLine.p2 += (transform.right * -h * 2f) + (transform.up * -v) + transform.forward * TP_Input.cc._capsuleCollider.radius;
        ...
        {
            if (TP_Input.enabled || !TP_Input.cc.animator || !dragInfo.inDrag || inClimbUp) return;
            if (climbJumpInput.GetButton() && !inClimbJump && input.magnitude > 0 && !TP_Input.cc.animator.GetCurrentAnimatorStateInfo(0).IsName(animatorStateHierarchy + ".ClimbJump"))
            {
                ...
                for (int i = 0; casts < 1f; i++)
                {
                    ...
                    {
                        var p = transform.position + Vector3.up * TP_Input.cc._capsuleCollider.height * casts;
        ...
        private void CheckClimbUp(bool ignoreInput = false)
        {
            ...
            if (!canMoveClimb && !inClimbUp && (climbUpConditions || ignoreInput))
            {
                ...
                var climbPoint = thicknessPoint + Vector3.down * (TP_Input.cc._capsuleCollider.height * 0.5f);
        ...
        protected virtual void EnterClimb()
        {
            ...
            RaycastHit hit;
            var climbUpConditions = TP_Input.cc.isGrounded && !Physics.Raycast(transform.position + Vector3.up * TP_Input.cc._capsuleCollider.height, Vector3.up, TP_Input.cc._capsuleCollider.height * 0.5f, obstacle) &&
                Physics.Raycast(transform.position + Vector3.up * (TP_Input.cc._capsuleCollider.height * climbUpHeight), transform.forward, out hit, 1f, draggableWall) && draggableTags.Contains(hit.collider.gameObject.tag);
        ...
        void CalculateMovementRotation()
        {
            ...
            centerLine.p2 += transform.forward * (TP_Input.cc._capsuleCollider.radius * 2);
            ...
            centerLine.p2 += (transform.right * lastPointDistanceH * -h * 2f) + (transform.up * lastPointDistanceVUp * -v) + transform.forward * TP_Input.cc._capsuleCollider.radius;
